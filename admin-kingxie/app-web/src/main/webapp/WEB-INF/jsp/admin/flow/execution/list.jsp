...
<div class="box box-primary">
    <div class="box-header with-border">
        <h3 class="box-title">运行中的执行实例列表</h3>
    </div>
    <div class="box-pane">
                ...
                <tr>
                    ...
                    <td>
                        <c:forEach items="${currentActivityMap[e.id]}" var="acid">
                            ...
                            <%--<a href="${ctx }/process/trace/view/${task.executionId}" target="_blank">
                                    &lt;%&ndash; 处理[调用活动] &ndash;%&gt;
                            ...
                            <a href="${ctx }/bpm/diagram-viewer/index.html?processDefinitionId=${task.processDefinitionId}&processInstanceId=${task.processInstanceId}" target="_blank">
                                    <%-- 处理[调用活动] --%>
                                <c:if test="${task.processDefinitionId != e.processDefinitionId}">
                                    <span title='引用了外部流程'>${definitions[task.processDefinitionId].name}</span><i style="margin-left: 0.5em;" class="icon-circle-arrow-right"></i>
                                </c:if>
